...
def generate_graph(
    ...
    """
    Preconditions:
    - virus_parameter = ["infection_rate", "initial_infected_count",
    ...
    - population_parameter = ["population_size", "house_density",
        "contact_density"] are keys in population_data
    ...
    while (_finished_nodes < num_nodes - int(house_density)):
        ...
        family_size = generate_number_normally(house_density, house_density / 2, 1)[0]
    ...
    for day in range(1, visualization_data.get("total_days", 30)):
        ...
        node_colors = [color for node, color in sorted(node_colors_dict.items())]

        nx.draw(graph, pos=positions, node_color=node_colors, with_labels=False, ax=ax, node_size=50)
...
if __name__ == "__main__":
    virus_data = {'infection_rate': 0.3, 'initial_infected_count': 6, 
                  'incubation_period': 5, 'death_rate': 0.1, 'recovery_days': 7}
    population_data = {'population_size': 50, 'house_density': 3.6, 'contact_density': 5}
